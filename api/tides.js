// Vercel Serverless Function - Stormglass proxy s letnim kešom
const STORE = {};
...
module.exports = async (req, res) => {
  ...
  if (!sgKey) {
    return res.status(500).json({ error: 'STORMGLASS_KEY ni nastavljen' });
  }
  ...
  try {
    ...
    return res.status(200).json({ extremes: dayData, date, cached: false, total: extremes.length });
